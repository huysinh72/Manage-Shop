...
{
	data = [];
    for(i = 0; i < list_invoice.length; i++)
    {
        em = {};
        em.Id = list_invoice[i].sequenceNo;
        em.Time = list_invoice[i].time;
        em.Customer = list_invoice[i].customerName;
        em.TotalMoney = list_invoice[i].salePriceTotal;
        em.PaymentType = paymentType[list_invoice[i].paymentMethod];
        em.Branch = list_invoice[i].branchName;
        em.Employee = list_invoice[i].employeeName;
        em.State = states[list_invoice[i].state-1]
        data.push(em);

    }
    var filename = '"Invoice report - ' + getCookie("shopName") + ' - ' + document.getElementById("datepickerStart").value + ' - ' + document.getElementById("datepickerEnd").value +'.xlsx"';
