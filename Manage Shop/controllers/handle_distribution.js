				...
				firebase.database().ref().child(Shop).child(shopId).child("branchStore").orderByChild("id").equalTo(this.selectedProduct.id)
					.once('value', snapshot => {
						snapshot.forEach(function(childSnapshot) {
							...
							table_distribution.row.add([countProduct,product.branchName, saleQuantity, product.quantity2, product.quantity1, product.quantity1+product.quantity2, createInputElementTableDistribution(countProduct, product.quantity2)]).draw();
					  		countProduct ++;
					  	});
					});
			}
