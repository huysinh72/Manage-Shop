...
function loadBranchData(startDate, endDate)
{
    ...
    once('value', snapshot => {
        ...
        if(products.length > 0)
        {
            ...
            for(i = 0; i < products.length; i++)
                table_product.row.add([count++, products[i].name, products[i].number, accounting.formatNumber(products[i].revenue), accounting.formatNumber(products[i].profit)]).draw();
        ...
        else
        {
            ...
            productProfitBar.setData(products);
...
var productProfitBar =  Morris.Bar({
